...
""" ---------------------------------------------------------------------------------
# Config: ProtoNet, Few-Rel, Conv1D, 5 Ways, 5 Shots
max_len = 128
embedding_dim = 50
init_vector = backbones.GloVeRC(embedding_dim=embedding_dim)
TRAIN_DATASET = datasets.FewRel(mode='train', max_len=max_len, vector_initializer=init_vector)
...
""" ---------------------------------------------------------------------------------
# Config: ProtoNet, Few-Rel, Conv1D, 10 Ways, 1 Shot
max_len = 128
embedding_dim = 50
init_vector = backbones.GloVeRC(embedding_dim=embedding_dim)
TRAIN_DATASET = datasets.FewRel(mode='train', max_len=max_len, vector_initializer=init_vector)
...
""" ---------------------------------------------------------------------------------
# Config: ProtoNet, Few-Rel, Conv1D, 10 Ways, 5 Shots
max_len = 128
embedding_dim = 50
init_vector = backbones.GloVeRC(embedding_dim=embedding_dim)
TRAIN_DATASET = datasets.FewRel(mode='train', max_len=max_len, vector_initializer=init_vector)
...
MODEL = paddle.nn.Sequential(position_emb, conv1d)
MODEL._full_name = 'glove50_cnn'
LR = 0.001  # paddle.optimizer.lr.ExponentialDecay(learning_rate=0.001, gamma=0.5)
OPTIMIZER = paddle.optimizer.Adam(learning_rate=LR, parameters=MODEL.parameters())
EPOCHS = 50
TEST_EPOCHS = 10
EPISODES = 1000
