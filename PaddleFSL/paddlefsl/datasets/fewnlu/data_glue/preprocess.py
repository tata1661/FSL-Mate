...
from paddlefsl.datasets.fewnlu.data_glue.task_pvps import PVP, PVPS
...
class Preprocessor(ABC):
    ...
    def __init__(self, wrapper, task_name, pattern_id: int = 0, if_pt = True):
        """
        ...
        self.pvp = PVPS[task_name](self.wrapper, pattern_id, if_pt)  # type: PVP
        self.label_map = {label: i for i, label in enumerate(self.wrapper.label_list)}
...
class MLMPreprocessor(Preprocessor):
    ...
    def get_input_features(self, example: InputExample, labelled: bool, priming: bool = False,
        ...
        padding_length = self.wrapper.max_seq_length - len(input_ids)
        ...
        assert len(input_ids) == self.wrapper.max_seq_length
        assert len(attention_mask) == self.wrapper.max_seq_length
        assert len(token_type_ids) == self.wrapper.max_seq_length
        assert len(block_flag) == self.wrapper.max_seq_length

        label = self.label_map[example.label] if example.label is not None else -100
        logits = example.logits if example.logits else [-1]

        if labelled:
            labels, label_idx = self.pvp.get_mask_positions(input_ids)
            mlm_labels = self.pvp.get_mask_labels(example.label)
        else:
            mlm_labels = [-1] * self.wrapper.max_seq_length
        ...
        return InputFeatures(input_ids=input_ids,
                             ...
                             logits=logits,
                             idx=example.idx,
                             block_flag=block_flag), label_idx